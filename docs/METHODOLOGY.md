...
Cvar queries use one ordered batch followed by a unique engine echo acknowledgement. Missing replies remain quality blockers; they no longer incur a five-second wait per hidden variable. A missing acknowledgement fails the capture. Readback overlaps camera settling, and the final demo-info request uses an acknowledgement instead of a fixed shutdown delay. Full GameInfo restarts and transaction restoration remain in place for every capture.
