...
The runner waits for a fresh game process and a run-specific Deadlock CSV. It waits for the engine's completed demo-signon message instead of a fixed 20-second delay. After replay warm-up it seeks back to the requested tick, pauses to establish the camera, records the latest complete logger timestamp and resumes. The analysis window is `[start_elapsed_s, start_elapsed_s + sample_s)`. It stops the game to finalize the file before hashing and parsing it. Startup, loading and warm-up are outside that interval. There is a small command/CSV sampling alignment error; this is not tick-exact frame capture or a deterministic engine timedemo.

A positive finite frame time is retained regardless of size. Long stalls must not disappear merely because they make a config look worse. Nonpositive, NaN, infinite and malformed rows are counted and disclosed; automated verdicts remain blocked when they occur. Missing logs, insufficient samples, backwards timestamps, mismatched per-frame coverage and incomplete windows fail the capture. No real-run fallback produces synthetic FPS or CPU data.
